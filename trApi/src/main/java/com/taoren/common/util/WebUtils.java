 ...
 * 网络工具类。
 * 
 * @author wangshuisheng copy from net
 *
 */
public abstract class WebUtils {
	...
	public static byte[] doPost(String url, String charset, Map<String, String> params, int connectTimeout, int readTimeout, Map<String, String> headerMap) throws IOException {
		String ctype = "application/x-www-form-urlencoded;charset=" + charset;
		HttpURLConnection conn = null;
		OutputStream out = null;
		byte[] rsp = null;
...
//			}
	...
	protected static byte[] getResponseAsByteArray(HttpURLConnection conn) throws IOException {
		...
		InputStream es = conn.getErrorStream();
		if (es == null) {
			return getStreamAsByteArray(conn.getInputStream(), charset);
		} else {
			String msg = getStreamAsString(es, charset);
			if (StringUtils.isEmpty(msg)) {
				throw new IOException(conn.getResponseCode() + ":" + conn.getResponseMessage());
	...
	private static byte[] getStreamAsByteArray(InputStream stream, String charset) throws IOException {
		try {
